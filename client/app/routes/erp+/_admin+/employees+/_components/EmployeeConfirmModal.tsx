...
export default function EmployeeConfirmModal({
  ...
  setSelectedEmployees: (employes: IEmployee[]) => void;
}) {
  ...
  useEffect(() => {
    if (bulkDeleteFetcher.data) {
      ...
      if (response.success) {
        toast.success(`Đã xóa ${selectedEmployees.length} Nhân sự thành công!`);
        setSelectedEmployees([]);
      } else {
        toast.error(response.error || 'Có lỗi xảy ra khi xóa Nhân sự.');
      }
      setIsDeleting(false);
    }
      ...
      >
        ...
        <p className='mb-6'>
          {selectedEmployees.length > 1
            ? `Bạn có chắc chắn muốn xóa ${selectedEmployees.length} Nhân sự này?`
